# Product CRUD - E-Commerce Spring Boot Application

A Spring Boot REST API to **Store / Update / Retrieve / Delete** product records
(`id`, `name`, `productsku`) backed by **MySQL**, documented with **Swagger (springdoc-openapi)**,
...
└── src
    ...
    └── test
        ├── java/com/example/productcrud
        │   ├── ProductCrudApplicationTests.java     # full integration test (H2)
        │   ├── controller/ProductControllerTest.java # @WebMvcTest
        │   └── service/ProductServiceImplTest.java   # Mockito unit test
...
### IntelliJ IDEA
1. `File → Open` → select the `product-crud` folder (the one containing `pom.xml`).
...
6. Run `ProductCrudApplication.java` (right-click → Run), or use the Maven side panel.

### Eclipse / Spring Tool Suite (STS)
1. `File → Import → Maven → Existing Maven Projects` → select `product-crud`.
...
4. Right-click `ProductCrudApplication.java → Run As → Spring Boot App`.

### VS Code
1. Install extensions: *Extension Pack for Java*, *Spring Boot Extension Pack*, *Lombok Annotations Support*.
2. Open the `product-crud` folder.
3. Use the Spring Boot Dashboard or `Run → Start Debugging` on `ProductCrudApplication`.
...
1. Install MySQL 8 locally, or run it via Docker:
   ```bash
   docker run --name mysql-product -e MYSQL_ROOT_PASSWORD=root -p 3306:3306 -d mysql:8.0
   ```
2. The app is configured (see `src/main/resources/application.properties`) to:
   - connect to `jdbc:mysql://localhost:3306/productdb`
   - auto-create the database (`createDatabaseIfNotExist=true`)
   - auto-create/update the `product` table (`spring.jpa.hibernate.ddl-auto=update`)
...
|-----------------------------------|----------------------------|--------|
| `ProductServiceImplTest`          | Unit (Mockito)             | create, get-by-id (found/not-found), get-all, update (found/not-found), delete (found/not-found) |
| `ProductControllerTest`           | Web slice (`@WebMvcTest` + MockMvc) | all 5 endpoints, success + validation (400) + not-found (404) paths |
| `ProductCrudApplicationTests`     | Integration (`@SpringBootTest` + H2) | Spring context loads; full create → read → update → delete lifecycle through real HTTP calls |
...
From inside the `product-crud` folder:

```bash
git init
git add .
git commit -m "Initial commit: Product CRUD Spring Boot app with Swagger and unit tests"
...
|--------|-------------------------------|---------------------------|-------------|
| POST   | `/api/v1/products`             | Create a product           | `{"name":"Wireless Mouse","productsku":"WM-BLK-001"}` |
| GET    | `/api/v1/products/{id}`        | Retrieve one product       | – |
| GET    | `/api/v1/products`              | Retrieve all products      | – |
| PUT    | `/api/v1/products/{id}`        | Update a product            | `{"name":"Wireless Mouse (2.4GHz)","productsku":"WM-BLK-002"}` |
...
curl -X POST http://localhost:8080/api/v1/products \
  ...
  -d '{"name":"Wireless Mouse","productsku":"WM-BLK-001"}'
...
curl -X PUT http://localhost:8080/api/v1/products/1 \
  -H "Content-Type: application/json" \
  -d '{"name":"Wireless Mouse (2.4GHz)","productsku":"WM-BLK-002"}'
...
### Error handling demo
- `GET /api/v1/products/999` (non-existent id) → `404 Not Found` with a JSON error body
  (`timestamp`, `status`, `error`, `message`, `path`).
- `POST /api/v1/products` with `{"name":"","productsku":"WM-BLK-001"}` → `400 Bad Request` with
...
- Add pagination to `GET /api/v1/products` if the dataset can grow large.
